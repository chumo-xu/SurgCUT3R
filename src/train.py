...
from dust3r.losses import *  # noqa: F401, needed when loading the model (现在包含MegaSAM)
...
def train(args):
    ...
    data_loader_test = {
        dataset.split("(")[0]: accelerator.prepare(build_dataset(
            dataset,
            args.batch_size,
            args.num_workers,
            accelerator=accelerator,
    ...
    # Initialize RAFT model for SelfSupervisedLoss
    printer.info(">> Initializing RAFT model for SelfSupervisedLoss...")
    ...
    raft_model_wrapper.load_state_dict(torch.load(raft_weight_path, map_location=device))
    
    raft_model = raft_model_wrapper.module
    raft_model.to(device)
    raft_model.eval()
    
    # Make raft_model available for eval()
    globals()['raft_model'] = raft_model
    printer.info("✅ RAFT model initialized successfully")
    ...
    for data_iter_step, batch in enumerate(
        ...
        with accelerator.accumulate(model):
            ...
            if tb_vis_img:
                ...
                # 🔥 只有当num_imgs_vis > 0时才进行可视化
                if args.num_imgs_vis > 0:
...
def test_one_epoch(
    ...
    if log_writer is not None:
        ...
        for k in range(len(batch)):
            ...
            # 🔥 添加缺失的图像键，用于可视化
            # 添加真实图像（从 B,C,H,W 转换为 B,H,W,C）
            loss_details[f"gt_img{k+1}"] = batch[k]["img"].permute(0, 2, 3, 1).detach()

            # 添加预测RGB图像（如果模型有RGB输出）
            if "rgb" in result["pred"][k]:
                loss_details[f"pred_rgb_{k+1}"] = result["pred"][k]["rgb"].detach()
            else:
                # 如果没有RGB预测，使用真实图像作为占位符
                loss_details[f"pred_rgb_{k+1}"] = batch[k]["img"].permute(0, 2, 3, 1).detach()

            # 添加掩码信息（如果存在）
            if "img_mask" in batch[k] and "ray_mask" in batch[k]:
                loss_details[f"img_mask_{k+1}"] = batch[k]["img_mask"].detach()
                loss_details[f"ray_mask_{k+1}"] = batch[k]["ray_mask"].detach()

            # 添加置信度信息（如果存在）
            if "conf_self" in result["pred"][k]:
                loss_details[f"self_conf_{k+1}"] = result["pred"][k]["conf_self"].detach()
            if "conf" in result["pred"][k]:
                loss_details[f"conf_{k+1}"] = result["pred"][k]["conf"].detach()
...
def get_vis_imgs_new(loss_details, num_imgs_vis, num_views, is_metric):
    ...
    # 添加调试信息和空列表检查
    print(f"[DEBUG] num_imgs_vis: {num_imgs_vis}, num_views: {num_views}, stride: {stride}")
    print(f"[DEBUG] gt_img_list lengths: {[len(sublist) for sublist in gt_img_list]}")

    # 检查是否有空列表，如果有则跳过可视化
    if any(len(sublist) == 0 for sublist in gt_img_list):
        print("[WARNING] 检测到空的图像列表，跳过可视化")
        return {}
    ...
    # 检查深度列表是否为空
    if any(len(sublist) == 0 for sublist in cross_pred_depth_list):
        print("[WARNING] 检测到空的深度列表，使用占位符")
        cross_pred_depth_list = [torch.zeros(1, 1) for _ in range(num_imgs_vis)]
        cross_gt_depth_list = [torch.zeros(1, 1) for _ in range(num_imgs_vis)]
        self_gt_depth_list = [torch.zeros(1, 1) for _ in range(num_imgs_vis)]
        self_pred_depth_list = [torch.zeros(1, 1) for _ in range(num_imgs_vis)]
    else:
