...
class C3VD_Multi(BaseMultiViewDataset):
    def __init__(self, *args, ROOT, **kwargs):
        ...
        # C3VD数据集不使用split分割，所以不需要检查split参数
    ...
    def _load_data(self):
        ...
        for scene in tqdm(os.listdir(root)):
            ...
            if not osp.exists(rgb_dir):
                print(f"Skipping {scene} (no rgb directory)")
                continue
            ...
            print(f"Loading scene {scene} with {num_imgs} images")
        ...
        print(f"Loaded {len(scenes)} scenes with total {len(images)} images")
        self.scenes = scenes
    ...
    def _get_views(self, idx, resolution, rng, num_views):
        ...
        for v, view_idx in enumerate(image_idxs):
            ...
            # C3VD深度单位是米，缩放20倍以匹配预训练权重的尺度
            depthmap = depthmap * 20.0  # 深度缩放20倍

            # 加载相机参数（位姿已经是cam2world格式）
            cam = np.load(osp.join(cam_dir, basename + ".npz"))
            camera_pose = cam["pose"].copy()  # 直接使用cam2world格式的位姿

            # 调试信息：验证位姿格式（仅在第一次加载时打印）
            if hasattr(self, '_pose_debug_printed') is False:
                print(f"[C3VD数据集] 位姿格式验证:")
                print(f"  位姿(cam2world)位移: {camera_pose[:3, 3]}")
                print(f"  位姿格式: cam2world (预处理时已转换) ✅")
