...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Starting frame index for subsequence processing.",
    )
    parser.add_argument(
        "--end_frame",
        type=int,
        default=None,
        help="Ending frame index for subsequence processing.",
    )
    parser.add_argument(
        "--no-viz",
        action="store_true",
        help="If set, disable the point cloud viewer.",
    )
...
def run_inference(args):
    ...
    # Handle subsequence processing
    if args.start_frame is not None and args.end_frame is not None:
        if args.start_frame < 0 or args.end_frame > len(img_paths) or args.start_frame >= args.end_frame:
            raise ValueError(f"Invalid frame range: start={args.start_frame}, end={args.end_frame}")
        print(f"Processing subsequence from frame {args.start_frame} to {args.end_frame}")
        img_paths = img_paths[args.start_frame:args.end_frame]
