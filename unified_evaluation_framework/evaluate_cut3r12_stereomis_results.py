...
class StereoMISResultsEvaluator:
    ...
    def __init__(self, verbose=True):
        ...
        # StereoMIS结果路径
        self.results_base = "/hy-tmp/hy-tmp/CUT3R/eval/cut3r1+cut3r2/stereomis_results"
    ...
    def get_sequence_paths(self, sequence_name):
        ...
        paths = {
            ...
            # 预测结果路径
            'pred_pose_file': f"/hy-tmp/hy-tmp/CUT3R/eval/test_stereomis_sequences/{sequence_name}/cut3r1+cut3r2.npz",
    ...
    def evaluate_single_sequence(self, sequence_name):
        ...
        try:
            ...
            pose_results = self.evaluator.evaluate_pose_only(
                ...
                # 位姿配置
                gt_pose_format='npz_dir',        # GT位姿是NPZ目录
    ...
    def evaluate_all_sequences(self, output_summary_dir=None):
        ...
        if output_summary_dir is None:
            output_summary_dir = "/hy-tmp/hy-tmp/CUT3R/eval/cut3r1+cut3r2/stereomis_evaluation_summary_pose_only"
    ...
    def _save_detailed_report(self, summary, report_file):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("StereoMIS数据集 CUT3R 1+2 模型评估报告 - (仅位姿)\n")
            f.write("=" * 80 + "\n\n")
    ...
    def _print_summary(self, summary):
        """打印评估总结"""
        print("\n" + "=" * 80)
        print("🎉 StereoMIS数据集 CUT3R 1+2 模型评估完成 - (仅位姿)")
        print("=" * 80)
...
def main():
    ...
    if args.sequence:
        ...
        if result:
            ...
            # 打印简要结果
            # The evaluator.print_complete_results method is no longer available
            # as the evaluation is now pose-only.
            # If specific pose metrics need to be printed, they would need to be
            # extracted from the result dictionary.
            # For now, we'll just print a generic message.
            print("🎉 位姿评估结果已保存到:", result['pose_evaluation']['output_dir'])
    else:
        # 评估所有序列
