...
class StereoMISResultsEvaluator:
    """StereoMIS结果评估器"""
    ...
    def __init__(self, verbose=True):
        ...
        # StereoMIS结果路径
        self.results_base = "/hy-tmp/hy-tmp/CUT3R/eval/endodac_stereomis_results/endodac_results"
        self.gt_base = "/hy-tmp/hy-tmp/CUT3R/dataset/stereomis_testset"
    ...
    def get_sequence_paths(self, sequence_name):
        ...
        Args:
            sequence_name: 序列名称，如 "StereoMIS_P2_1_2"
        ...
        paths = {
            ...
            # GT数据路径  
            'gt_depth_dir': f"{self.gt_base}/{sequence_name}/depth",
            'gt_pose_dir': f"{self.gt_base}/{sequence_name}/cam",
    ...
    def validate_sequence_paths(self, paths):
        ...
        required_paths = [
            ...
            ('gt_depth_dir', paths['gt_depth_dir']),
            ('gt_pose_dir', paths['gt_pose_dir'])
        ]
    ...
    def evaluate_single_sequence(self, sequence_name):
        ...
        try:
            ...
            results = self.evaluator.evaluate_complete(
                gt_depth_path=paths['gt_depth_dir'],
                pred_depth_path=paths['pred_depth_dir'],
                gt_pose_path=paths['gt_pose_dir'],
                pred_pose_path=paths['pred_pose_file'],
                output_dir=paths['output_dir'],
                # 深度配置
                gt_depth_format='npy_dir',       # GT深度是NPY目录
                pred_depth_format='npy_dir',     # 预测深度是NPY目录
                gt_depth_unit='m',               # 单位：米
                pred_depth_unit='m',             # 单位：米
                # 位姿配置
                gt_pose_format='npz_dir',        # GT位姿是NPZ目录（每帧一个文件）
                pred_pose_format='npz_file',     # 预测位姿是NPZ文件
                gt_pose_unit='m',                # 单位：米
                pred_pose_unit='m',              # 单位：米
                # 评估配置
                depth_mode='global',             # 只使用全局深度评估方法
                # 可视化配置
                sequence_name=sequence_name
            )
    ...
    def evaluate_all_sequences(self, output_summary_dir=None):
        ...
        if output_summary_dir is None:
            output_summary_dir = "/hy-tmp/hy-tmp/CUT3R/eval/cut3r_inference_results/stereomis_evaluation_summary"
        
        os.makedirs(output_summary_dir, exist_ok=True)
        
        print("🚀 开始评估所有StereoMIS序列")
        print("=" * 70)
        ...
        # 保存总结果
        summary_file = os.path.join(output_summary_dir, 'stereomis_evaluation_summary.json')
        with open(summary_file, 'w', encoding='utf-8') as f:
            json.dump(summary, f, indent=2, ensure_ascii=False, default=str)
        
        # 保存详细报告
        report_file = os.path.join(output_summary_dir, 'stereomis_evaluation_report.txt')
    ...
    def _save_detailed_report(self, summary, report_file):
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("StereoMIS数据集CUT3R模型评估报告 - 统一评估框架\n")
            f.write("=" * 80 + "\n\n")
            ...
            if summary['average_metrics']['depth']:
                ...
                f.write("🎯 平均评估结果:\n")
                f.write("-" * 50 + "\n")
                f.write("深度指标 (单位说明: sq_rel和rmse显示为毫米mm):\n")
                f.write(f"  abs_rel:  {depth_avg['abs_rel']:.4f}\n")
                f.write(f"  sq_rel:   {depth_avg['sq_rel']*1000:.6f} mm\n")
                f.write(f"  rmse:     {depth_avg['rmse']*1000:.4f} mm\n")
            ...
            for depth_metric in summary['per_sequence_metrics']['depth']:
                ...
                f.write(f"{seq}:\n")
                f.write(f"  深度: abs_rel={depth_metric['abs_rel']:.4f}, sq_rel={depth_metric['sq_rel']*1000:.6f}mm, rmse={depth_metric['rmse']*1000:.4f}mm, a1={depth_metric['a1']:.4f}\n")
    ...
    def _print_summary(self, summary):
        """打印评估总结"""
        print("\n" + "=" * 80)
        print("🎉 StereoMIS数据集CUT3R模型评估完成 - 总结报告")
        print("=" * 80)
        ...
        if summary['average_metrics']['depth']:
            ...
            print(f"\n🎯 平均性能指标:")
            print(f"  深度估计 (单位说明: sq_rel和rmse显示为毫米mm):")
            print(f"    abs_rel:  {depth_avg['abs_rel']:.4f}")
            print(f"    sq_rel:   {depth_avg['sq_rel']*1000:.6f} mm")
            print(f"    rmse:     {depth_avg['rmse']*1000:.4f} mm")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='StereoMIS结果评估工具')
    ...
    # 初始化评估器
    evaluator = StereoMISResultsEvaluator(verbose=args.verbose)
